...
let reducer = (state: state, action: action): ReludeReact.Reducer.update<action, state> =>
  switch action {
  | FetchAnimals =>
    UpdateWithIO(
      {...state, animalsResult: state.animalsResult |> AsyncResult.toBusy},
      API.fetchAnimals |> IO.bimap(a => FetchAnimalsSuccess(a), e => FetchAnimalsError(e)),
    )
...
module AnimalsLoading = {
  @react.component
  let make = () => <div> {React.string("Loading animals...")} </div>
}
...
module AnimalsTable = {
  ...
  let make = (~animals: list<Animal.t>, ~send: action => unit) => {
    let _ = send // TODO
    <div> {React.string("Animals: " ++ string_of_int(List.length(animals)))} </div>
  }
}

module AnimalsError = {
  @react.component
  let make = (~error: Error.t) => <div> {React.string(Error.show(error))} </div>
}

module AnimalsResult = {
  @react.component
  let make = (~result: AsyncResult.t<list<Animal.t>, Error.t>, ~send) =>
    result |> ReludeReact.Render.asyncResultByValueLazy(
      _ => <AnimalsLoading />,
